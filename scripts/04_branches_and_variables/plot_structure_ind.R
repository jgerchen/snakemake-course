...
n_inds=nrow(pops)
#grep like this somehow...
#system("grep -A ",as.character(n_inds+2)," 'Inferred ancestry of individuals:' ",input_file," | tail -n +3")

struct_results_txt<-system(paste("grep -A ",as.character(n_inds+1)," 'Inferred ancestry of individuals:' ",input_file," | tail -n +3"), intern=TRUE)
results<-read.table(textConnection(struct_results_txt))
