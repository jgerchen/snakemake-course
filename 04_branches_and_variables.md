...
We now want to do amore complex slimulation which starts with two completely separate populations with 500 indviduals each. After 5000 generations a third population with the size of 200 splits off from the second population. There is unidirectional gene flow from the second to the third population with one migrant per generation. After 10000 generations we randomly sample 20 individuals from each population and save the results as a VCF file again. We then want to take the output of this slimulation and analyze it using two different approaches: a principal component analysis on genotypes and using the bayesian clustering algorithm Structure.

To make this happen, you have to combine the following parts into a new workflow:

+ [The SLiM script](scripts/04_branches_and_variables/04_slimulation.slim), which defines how our slimulation runs. This time it will output a vcf file called **04_slim_output.vcf**. You can use the same conda environment for Slim as last time

+ [This python script](scripts/04_branches_and_variables/extract_table_from_vcf.py), which extracts sample names from a vcf file generated by SLiM (-v) and outputs a table (-o), which assigns the population and ploidy (2) for each sample. It is pure python and doesn't have any additional dependencies and it is run in the following way:

```
python3 extract_table_from_vcf.py -v vcf_file.vcf -o output_table.tsv
```

+ [This python script](scripts/04_branches_and_variables/vcf_to_structure.py) from my Github, which converts a vcf file into Structure input. It is pure python and doesn't have any additional dependencies, but it requires the vcf file (-v) and the table generated by the script above (-p). It will generate three files: **a mainparams file** (-s), **an extraparams file** (-e) and the **actual input file for structure** (-o). Also you have to set the maximum ploidy in your dataset (-m), which is 2. It is run in the following way:

```
python3 vcf_to_structure.py -v vcf_file.vcf -o output.struct -p pop_table.tsv -m 2 -s mainparams -e extraparams
```

+ [**Structure**](https://web.stanford.edu/group/pritchardlab/structure.html). Everyone's favourite popgen clustering tool for 25 years! You can install the latest version (2.34) from the [bioconda channel](https://anaconda.org/channels/bioconda/packages/structure/overview). For running structure from the command line (without a graphical frontend) we just use the command 'structure', but we need two configuration files (**mainparams** and **extraparams**) and an input file. When the configuration files are named **mainparams** and **extraparams** struture will automatically detect them when it is run in the same folder, otherwise you can also explicitely supply them to structure with the -m and -e parameters. The script above that generated all three files should automatically include the name of the input file in the **mainparams** file, but you can also set it explicitely with the -i parameter. You should set the name of your output file with the -o parameter. **However, note that structure will add the letters \_f to the end of the file name of your putput file**. In addition, we would like to tell structure to assume three clusters, which we can set with the -K parameter. 

+ [This R script used for plotting our Structure results](scripts/04_branches_and_variables/plot_structure_ind.R). It depends on R and requires the [viridis R package](https://anaconda.org/channels/conda-forge/packages/r-viridis/overview), which has a conda package with the name **r-viridis** on the conda forge channel. Its parameters are the structure output file, the population table we generated above and the pdf output file. It is run in the following way:

```
Rscript plot_structure_ind.R structure_output pop_table.tsv structure_plot.pdf
```

+ [This R script from my Github](https://github.com/jgerchen/polyintro/blob/main/workflow/scripts/PCA/make_PCA.r), which builds a PCA from the VCF file generated by SLiM. It depends on R and requires the vcfR package (which you already installed in the previous session) and [adegenet](https://anaconda.org/channels/bioconda/packages/r-adegenet/overview), which can be installed from the bioconda channel. Its parameters are the vcf file generated by SLiM, the population table we generated above, an output table with the location of individual samples across PCA axes, an output table with eigenvalues and a plot of the PCA results. 

```
Rscript make_PCA.r input.vcf pop_map.tsv pca_PCs.tsv pca_Eigen.tsv pca_plot.pdf
```

For the last two R scripts you can add the additional R packages to your previously generated R environment and use it for both rules.

Each of these components should get its own rule and the plot of the Structure results and the PCA results should not depend on each other. Before you actually run it **do a dry run and generate a plot of the DAG for each of the output files**.

> [!important]
> We will add more features to this workflow in the next parts, so please take the time you need to finish this part, ask for help if you get stuck somewhere and don't delete your Snakefile.
