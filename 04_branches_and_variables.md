...
+ This python script, which extracts sample names from a vcf file and outputs a table, which assigns the population and ploidy for each sample. It is pure python and doesn't have any additional dependencies and it is run in the following way:

+ This python script from my Github, which converts a vcf file into Structure output. It is pure python and doesn't have any additional dependencies, but it requires the vcf file and the table generated by the script above. It will generate three files: **mainparams**, **extraparams** and the output structure file you define. It is run in the following way:

+ Structure

+ This R script used for plotting our Structure results

+ This R script from my Github, which builds a PCA. 
